...
module('Acceptance | fr/slide 12', function(hooks) {
  setupApplicationTest(hooks);

  test('visiting /fr/slide12-fr', async function(assert) {
    await visit('/fr/slide12-fr');
  ...
  test('should have correct meta', async function(assert) {
    ...
    assert.strictEqual(
      ...
      'Slide 12 Title French | Rapport Annuel du CIGI 2019',
    );
    assert.strictEqual(
      document.querySelector('head meta[property="og:description"]').getAttribute('content'),
      'Slide 12 Description French',
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      ...
      'https://www.cigionline.org/interactives/2019annualreport/fr/slide12-fr',
    );
    assert.strictEqual(
  ...
  test('should have correct social links', async function(assert) {
    ...
    assert.dom('.social-2-btn').hasAttribute('href', 'https://twitter.com/intent/tweet?status=Rapport+Annuel+du+CIGI+2019+https://www.cigionline.org/interactives/2019annualreport/fr/slide12-fr');
    assert.dom('.social-3-btn').hasAttribute('href', 'https://www.linkedin.com/shareArticle?mini=true&url=https://www.cigionline.org/interactives/2019annualreport/fr/slide12-fr');
  });
  ...
  test('should have correct slide content', async function(assert) {
    ...
    assert.dom('.slide-content .slide-title h1').containsText('Slide 12 Title French');
    ...
    assert.dom('.slide-content .content-links a:nth-of-type(1) .content-copy p').containsText('Slide 12 Link 1 French');
    assert.dom('.slide-content .content-links a:nth-of-type(2) .content-copy h4').containsText('Paper Series');
    assert.dom('.slide-content .content-links a:nth-of-type(2) .content-copy p').containsText('Slide 12 Link 2 French');
    assert.dom('.slide-content .content-links a:nth-of-type(3) .content-copy h4').containsText('Article');
    assert.dom('.slide-content .content-links a:nth-of-type(3) .content-copy p').containsText('Slide 12 Link 3 French');
    assert.dom('.slide-content .content-links-mobile').exists();
    assert.dom('.slide-content .content-links-mobile a').exists({ count: 3 });
    assert.dom('.slide-content .content-links-mobile a:nth-of-type(1)').containsText('Slide 12 Link 1 French');
    assert.dom('.slide-content .content-links-mobile a:nth-of-type(2)').containsText('Slide 12 Link 2 French');
    assert.dom('.slide-content .content-links-mobile a:nth-of-type(3)').containsText('Slide 12 Link 3 French');
    assert.dom('.slide-content .paragraphs').exists();
    assert.dom('.slide-content .paragraphs p').exists({ count: 3 });
    assert.dom('.slide-content .paragraphs p:nth-of-type(1)').containsText('Slide 12 Paragraph 1 French');
    assert.dom('.slide-content .paragraphs p:nth-of-type(2)').containsText('Slide 12 Paragraph 2 French');
    assert.dom('.slide-content .paragraphs p:nth-of-type(3)').containsText('Slide 12 Paragraph 3 French');
    ...
    assert.dom('.background-image .hover-reveal .quote h3.hover-reveal-quote').containsText('Slide 12 Quote French');
    assert.dom('.background-image .hover-reveal .quote h4.hover-reveal-quote-source').containsText('Slide 12 Source French');
    ...
    assert.dom('.footer .footer-description').containsText('Slide 12 Photo Credit French');
  });

  test('should transition to /fr/slide11-fr on scroll-arrow-up-btn click', async function(assert) {
    await visit('/fr/slide12-fr');
  ...
  test('should transition to /fr/slide13-fr on scroll-arrow-down-btn click', async function(assert) {
    await visit('/fr/slide12-fr');
  ...
  test('should transition to /en/slide12-en on intl-link click', async function(assert) {
    await visit('/fr/slide12-fr');

...
export default Route.extend(GoogleAnalyticsMixin, ResetScrollMixin, {
  ...
  afterModel() {
    const title = `${get(this, 'intl').t('multilateralInstitutionsMarkATumultuousYear.title')} | ${get(this, 'intl').t('title')}`;
    set(this, 'headData.title', title);
    set(this, 'headData.description', get(this, 'intl').t('multilateralInstitutionsMarkATumultuousYear.description'));
    set(this, 'headData.url', `${ENV.host}${ENV.rootURL}multilateral-institutions-mark-a-tumultuous-year/`);
    const backgroundImage = get(this, 'backgroundImage').getSlideBackgroundImage('multilateral-institutions-mark-a-tumultuous-year');
    if (backgroundImage && backgroundImage.ogUrl) {
      set(this, 'headData.image', backgroundImage.ogUrl);
    }

    // Load next route background
    const nextRoute = get(this, 'routeOrder').getNextRoute('multilateral-institutions-mark-a-tumultuous-year');
    if (nextRoute) {
